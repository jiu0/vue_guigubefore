          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="supSaleAttr in spuSaleAttrList" :key="supSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active:item.isChecked==='1'}" v-for="item in supSaleAttr.spuSaleAttrValueList" 
                :key="item.id" @click="changeActive(item,supSaleAttr.spuSaleAttrValueList)">{{ item.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    methods:{
     ...
     async addShopCart(){
         // 1 发请求  
         // 需要知道 请求是否发成功  成功 进行路由跳转 失败 给用户提示  产品信息也要带过去
         try{
